...
import { SERVE_TIME_BONUS_FRACTION } from '../state/gameReducer'
...
export default function OrderTicket({ order, orderNumber, simple = false, isGlitched = false }: Props) {
  ...
  const actualReward = Math.round(recipe.reward + Math.max(0, Math.round(recipe.reward * SERVE_TIME_BONUS_FRACTION * (order.patienceLeft / order.patienceMax))))
